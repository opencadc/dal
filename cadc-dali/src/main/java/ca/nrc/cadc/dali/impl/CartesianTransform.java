...
/**
 * Utility code to provide a 3D cartesian transform that "rotates" shapes around
 * the unit sphere so they are far away from the meridian and poles.
 * 
 * @author pdowler
 */
public class CartesianTransform {
    ...
    private static CartesianTransform getTransform(double[] cube, boolean force) {
        double x1 = cube[0];
        double x2 = cube[1];
        double y1 = cube[2];
        double y2 = cube[3];
        ...
        return new CartesianTransform(); // no-op
    }
